...
def selectNGramModel(models, sentence):
    ...
    if models[0].trainingDataHasNGram(sentence):
        ...
        return models[1]
    elif models[2].trainingDataHasNGram(sentence):
...
def generateLyricalSentence(models, desiredLength):
    ...
    while sentenceTooLong(desiredLength, currentLength) == False:
        ...
        if modelType == models[0]:
            newToken = models[0].getNextToken(sentence)
            if newToken == '$:::$':
                break
            else:
            ...
            newToken = models[1].getNextToken(sentence)
            if newToken == '$:::$':
                break
            else:
                sentence.append(newToken)
            ...
            newToken = models[2].getNextToken(sentence)
            if newToken == '$:::$':
                break
            else:
                sentence.append(newToken)
                currentLength += 1
    return sentence[2:currentLength]
...
def generateMusicalSentence(models, desiredLength, possiblePitches):
    ...
    while sentenceTooLong(desiredLength, currentLength) == False:
        modelType = selectNGramModel(models, sentence)
        if modelType == models[0]:
            newToken = models[0].getNextNote(sentence, possiblePitches)
            if newToken == '$:::$':
                break
            else:
                sentence.append(newToken)
        elif modelType == models[1]:
            newToken = models[1].getNextNote(sentence, possiblePitches)
            if newToken == '$:::$':
                break
            else:
                sentence.append(newToken)
        else:
            newToken = models[2].getNextNote(sentence, possiblePitches)
            if newToken == '$:::$':
                break
            else:
                sentence.append(newToken)
    return sentence[2:currentLength + 1]
...
def runMusicGenerator(models, songName):
    ...
    possiblePitches = KEY_SIGNATURES[random.choice]
